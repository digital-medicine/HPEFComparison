...
for filename in $filenames; do
	...
	npy="$out"npy/
	out_final=$(dirname "${filename_no_ending/vid/csv_rtmlib}.csv")/
	if [ ! -d "$out" ]; then
	    mkdir -p "$out"
	fi
	if [ ! -d "$out_final" ]; then
	...
	start=$(date +%s.%N)
	python rtmo_demo.py --video-path $filename --out-video-root $out
	end=$(date +%s.%N)
	...
	python rtmlib_annotate_data.py -npy $npy -p $filename -time $output_time -out $out_final
done
