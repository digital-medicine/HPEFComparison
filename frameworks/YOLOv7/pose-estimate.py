...
# Changed
def run(poseweights="yolov7-w6-pose.pt",source="football1.mp4",npy_out_dir='out_npy/',device='cpu',view_img=False,
    ...
    if (cap.isOpened() == False):   #check if videocapture not opened
        ...
        # Changed
        out_video_name = 'out/'+f"{source.split('/')[-1].split('.')[0]}"
        ...
        while(cap.isOpened): #loop until cap opened or video not complete
            ...
            if ret: #if success is true, means frame exist
                ...
                # Changed
                np.save(npy_out_dir+str(frame_count)+'.npy', output_data[0].cpu())
                ...
                np.save(npy_out_dir+str(frame_count)+'.npy', '0')
                frame_count += 1
                break
...
def parse_opt():
    ...
    # Changed
    parser.add_argument('--npy_out_dir', type=str, default='out_npy/', help='output directory for npy files')
